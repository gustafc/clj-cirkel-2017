...
;
; Once the data has been generated, you also need to create a checksum of that data. Calculate the checksum only for the
; data that fits on the disk, even if you generated more data than that in the previous step.
;
...
; the next checksum character is a 0. This should produce a new string which is exactly half as long as the original. If
; the length of the checksum is even, repeat the process until you end up with a checksum with an odd length.
;
; For example, suppose we want to fill a disk of length 12, and when we finally generate a string of at least length 12,
...
;
;     Next, we start calculating the checksum; after one round, we have 0111110101, which 10 characters long (even), so
;     we continue.
;
...
(defn render-bits
  {:test #(is (= "0100" (render-bits (parse-bits "0100"))))}
  [bits]
  (apply str (map {true "1" false "0"} bits)))

(defn generate-bits
  {:test #(letfn [(generate-bit-str [s n] (render-bits (generate-bits (parse-bits s) n)))]
            (is (= "0100" (generate-bit-str "0100" 4)))
            (is (= "01" (generate-bit-str "0100" 2)))
            ;     1 becomes 100.
            (is (= "100" (generate-bit-str "1" 3)))
            ;     0 becomes 001.
            (is (= "001" (generate-bit-str "0" 3)))
            ;     11111 becomes 11111000000.
            (is (= "11111000000" (generate-bit-str "11111" 11)))
            ;     111100001010 becomes 1111000010100101011110000.
            (is (= "1111000010100101011110000" (generate-bit-str "111100001010" 25)))
            ; Example data, before checksum
            (is (= "10000" (generate-bit-str "10000" 5)))
            (is (= "10000011110" (generate-bit-str "10000" 11)))
...
(defn checksum-from-seed
  ...
  [seed size]
  (->> (generate-bits (parse-bits seed) size)
       (calculate-checksum)
       (render-bits)
       ))

(def puzzle-input "01000100010010111")
