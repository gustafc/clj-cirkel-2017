...
; repeated, a marker is added to the file, like (10x2). To decompress this marker, take the subsequent 10 characters and
; repeat them 2 times. Then, continue reading the file after the repeated data. The marker itself is not included in the
; decompressed output.
;
...
(def input (->> (clojure.java.io/file "./resources/advent/day09_explosions.txt")
                (slurp)
                (str)))
    ...
    (if (empty? s)
      ...
      (if-let [[rep-instruction rep-len reps] (re-find #"^\((\d+)x(\d+)\)" s)]
        (let [rep-len (read-string rep-len)
              ...
              skip-to (+ rep-len (count rep-instruction))]
          (recur (subs s skip-to) (+ exploded-len (* rep-len reps))))
